...
export interface AppointmentProps {
  doctors?: Array<{ name: string; role?: string; slug?: string }>;
  contact?: {
    phone?: string;
    whatsapp?: string;
    morningOpd?: string;
...
export default function AppointmentClientView({ doctors, contact }: AppointmentProps) {
  ...
  const morningSlot = contact?.morningOpd || "Morning (10:00 AM – 2:00 PM)";
  const eveningSlot = contact?.eveningOpd || "Evening (5:30 PM – 8:00 PM)";
  const sundaySlot = contact?.sundayOpd || "Sunday (11:00 AM – 1:00 PM)";
        ...
        <div style={{ maxWidth: "1200px", margin: "0 auto" }}>
          ...
          <div style={{ maxWidth: "780px" }}>
            ...
            <p style={{ fontSize: "16px", color: "rgba(255,255,255,0.85)", lineHeight: 1.75 }}>
              Reserve your consultation with our senior specialists at our Sector 33C clinic in Chandigarh to avoid waiting delays.
            </p>
          </div>
        </div>
      </section>
              ...
              <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "22px" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: "16px" }}>
                  ...
                  <div>
                    ...
                    >
                      <option value={morningSlot}>{morningSlot}</option>
                      <option value={eveningSlot}>{eveningSlot}</option>
                      <option value={sundaySlot}>{sundaySlot}</option>
                    </select>
                  </div>
                </div>
                ...
                <div style={{ fontSize: "12px", color: "var(--text-muted)", textAlign: "center" }}>
                  💡 Need urgent assistance? You can also call the reception directly at <a href={`tel:${phoneClean}`} style={{ color: "var(--navy)", fontWeight: 600 }}>{phoneDisplay}</a> or message on <a href={`https://wa.me/${whatsappClean}`} target="_blank" rel="noopener noreferrer" style={{ color: "var(--navy)", fontWeight: 600 }}>WhatsApp</a>.
                </div>
              </form>
            </div>
          )}
